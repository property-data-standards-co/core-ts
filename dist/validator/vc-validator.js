...
export class VcValidator {
    ...
    async validateSignature(vc, options) {
        if (!vc.proof) {
        ...
        try {
            ...
            const vm = didDoc.verificationMethod?.find(m => m.id === vmUri || m.id === `#${vmUri.split('#')[1]}`);
            if (!vm) {
                return {
                    passed: false,
            ...
            return {
                passed: false,
                errors: [`Signature verification error: ${err.message}`],
            };
        }
    }
    async validateTir(vc, options, warnings) {
        ...
        try {
            ...
            return {
                passed: false,
                errors: [`TIR verification error: ${err.message}`],
            };
        }
    }
    async validateStatus(vc, options) {
        ...
        try {
            const isRevoked = await checkStatus(vc.credentialStatus.statusListCredential, parseInt(vc.credentialStatus.statusListIndex, 10), options.fetchFn);
            return {
                passed: !isRevoked,
                errors: isRevoked
            ...
            return {
                passed: false,
                errors: [`Status check error: ${err.message}`],
            };
        }
    }
}
//# sourceMappingURL=vc-validator.js.map
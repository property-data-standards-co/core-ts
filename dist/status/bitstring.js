 ...
 */
export function createStatusList(size = MIN_BITSTRING_SIZE) {
    if (size < MIN_BITSTRING_SIZE) {
        throw new Error(`Status list must be at least ${MIN_BITSTRING_SIZE} bits (16KB). Got ${size}.`);
    }
    if (size % 8 !== 0) {
 ...
 */
export function encodeStatusList(bitstring) {
 ...
 */
export function setBit(bitstring, index) {
    const byteIndex = Math.floor(index / 8);
    const bitIndex = index % 8;
    if (byteIndex >= bitstring.length) {
        throw new Error(`Index ${index} out of range for status list of size ${bitstring.length * 8}`);
    }
 ...
 */
export function revokeCredential(bitstring, index) {
 ...
 */
export async function checkStatus(statusListCredentialUrl, statusListIndex, fetchFn = globalThis.fetch) {
    const response = await fetchFn(statusListCredentialUrl, {
        headers: { 'Accept': 'application/json' },
        signal: AbortSignal.timeout(10_000),
    });
    if (!response.ok) {
        throw new Error(`Failed to fetch status list from ${statusListCredentialUrl}: HTTP ${response.status}`);
    }
